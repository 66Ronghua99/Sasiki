...
- **Sandbox observe->compact->refine chain 已推进到 skill-loaded refine，但 redirect resilience 仍是已知问题**: full-access 环境下，direct `refine` run `20260327_142923_316` 已确认浏览器启动和真实 TikTok 客服检查可完成；sandbox `selfcheck` 现在也能稳定跑过 `observe -> sop-compact`，compact run `20260327_145330_192` 已产出 durable SOP skill `tiktok-shop-check-inbox-messages`，并且后续 refine run `20260327_145552_964` 已实际通过 `skill.reader` 读入该 skill。当前剩余 blocker 不是 prompt 是否提供 homepage URL，而是 agent 直跳 `/chat/inbox/current` 时在当前站点会观测到被落回 homepage / message-center 入口，因此 refine 仍需更稳的 runtime redirect-recovery 策略（例如观测到 path mismatch 后主动改走页面内客服入口），这应由 agent/tooling 自恢复承担，不应要求 user prompt 明示。
