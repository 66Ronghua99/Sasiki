...
export interface PiAgentLoopAgent {
  setSystemPrompt(prompt: string): void;
  setThinkingLevel(level: PiAgentLoopConfig["thinkingLevel"]): void;
...
export interface PiAgentLoopRunOptions {
...
export class PiAgentLoop {
  private readonly config: PiAgentLoopConfig;
  private readonly tools: ToolClient;
  private readonly logger: Logger;
  private readonly toolAdapter: PiAgentToolAdapter;
  private runtimeTelemetry: RuntimeRunTelemetry | null = null;
  private agent: PiAgentLoopAgent | null = null;
  private activeProgress: PiAgentLoopProgressSnapshot | null = null;
  private latestProgress: PiAgentLoopProgressSnapshot = this.emptyProgressSnapshot();

  constructor(config: PiAgentLoopConfig, tools: ToolClient, logger: Logger) {
    this.config = config;
    this.tools = tools;
    this.logger = logger;
    this.toolAdapter = new PiAgentToolAdapter(tools);
  }
  ...
  async run(task: string, options?: PiAgentLoopRunOptions): Promise<AgentRunResult> {
  ...
  snapshotProgress(options?: PiAgentLoopSnapshotOptions): PiAgentLoopProgressSnapshot {
  ...
  private requireAgent(): PiAgentLoopAgent {
    if (!this.agent) {
  ...
  private emptyProgressSnapshot(): PiAgentLoopProgressSnapshot {
    return {
      steps: [],
      mcpCalls: [],
      assistantTurns: [],
  ...
  private cloneProgressSnapshot(snapshot: PiAgentLoopProgressSnapshot): PiAgentLoopProgressSnapshot {
    return {
