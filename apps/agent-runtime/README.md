...
- `sop-compact` now follows `evidence -> agent abstraction -> validation -> execution_guide`.
- If semantic/structured abstraction is unavailable, compact falls back to a conservative asset and should remain `needs_clarification` instead of pretending to be replay-ready.
