...
- **Refine tool surface unification Tasks 1-6 are complete**: Task 1 froze the current bridge/bootstrap/facade regression behavior, Task 2 introduced refine-owned `tools/` core abstractions, Task 3 added provider/hook/lifecycle scaffolding, Task 4 migrated the runtime-facing refine tools into first-class definitions registered through a production-side runtime registry seam, Task 5 migrated the core browser-facing refine tools into first-class definitions registered through a production-side browser registry seam, and Task 6 migrated screenshot/file-upload into first-class browser definitions while preserving screenshot capability negotiation plus file-upload compatibility behavior. The live refine runtime path still runs through the existing adapter-centric client, which keeps the migration incremental.
...
- The active next step is to execute Task 7 of the refine tool surface unification plan and rebuild `RefineReactToolClient` into a compatibility facade over the new surface, then remove the old adapter-centric registry path.
