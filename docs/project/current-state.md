...
- **Refine tool surface unification Tasks 1-3 are complete**: Task 1 froze the current bridge/bootstrap/facade regression behavior, Task 2 introduced refine-owned `tools/` core abstractions plus moved the 12-tool ordering contract out of `domain/`, and Task 3 added provider/hook/lifecycle scaffolding with regression coverage for bridge observer adaptation and monotonic observation refs under repeated provider observations. The live refine runtime path still runs through the existing adapter-centric client, which keeps the migration incremental.
...
- The active next step is to execute Task 4 of the refine tool surface unification plan and start moving runtime-facing refine tools into first-class definitions.
