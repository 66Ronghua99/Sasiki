...
- The active follow-up in this worktree is now proving the tool-path value of page-level retrieval cues on top of the stabilized TikTok customer-service baseline: the next step is to run a fresh TikTok refine e2e, inspect whether the agent actually consumes `observe.page.pageKnowledge`, and then decide whether to remove prompt-time knowledge injection or instead keep pushing query-surface / inbox-summary expansion.
