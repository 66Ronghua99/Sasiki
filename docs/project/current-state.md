...
- The active follow-up in this worktree is now query-surface and task-facing inbox-summary design on top of the stabilized TikTok customer-service baseline: the next step is to run a fresh TikTok refine e2e, inspect how `observationReadiness` behaves in empty inbox / new tab / partial page states, and then freeze which additional deterministic text buckets should enter `observe.query` versus remain adapter-owned summaries.
