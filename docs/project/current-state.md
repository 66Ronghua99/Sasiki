...
- **SOP skill refine bootstrap + tool slice is complete in this worktree**: `refine` now accepts optional `--skill <name>`, bootstrap loads the persistent SOP skill frontmatter catalog by default, explicit skill references fail fast if missing, and start prompts receive the available-skill bank plus requested skill name without inlining full skill markdown bodies. Detailed SOP loading now lives behind the narrow `skill.reader` refine tool, and `runtime-composition-root.ts` remains the only concrete assembly owner for `SopSkillStore`.
- **SOP skill compact persistence + discovery slice is complete in this worktree**: `sop-compact` now writes durable SOP skill markdown documents under `~/.sasiki/skills/` only after explicit `ready_to_finalize` convergence, `sop-compact list` exposes the frontmatter catalog through the shell-owned composition path, and `SopSkillStore` now enforces explicit failure for invalid persisted skill states such as missing skill documents, empty bodies, name mismatches, or invalid stable handles.
- **SOP skill compact->refine closeout verification is complete in this worktree**: fresh `lint`, `test`, `typecheck`, `build`, and `hardgate` all pass after the final doc sync; built CLI dry check `node apps/agent-runtime/dist/index.js sop-compact list` returns `[]` for the empty local skill-store baseline; strict reviewer recheck under `harness:refactor` reported no current findings; and the updated structural/focused tests remain the active `harness:lint-test-design` proof for the decoupled skill seams. Fresh hardgate evidence is `artifacts/code-gate/2026-03-27T04-49-22-654Z/report.json`.
- **The sandbox handoff bugfix slice is complete in this worktree, while redirect recovery remains open**: under full-access runtime conditions, direct refine run `20260327_142923_316` proved Chrome launch plus real TikTok customer-service checking works end-to-end; sandbox selfcheck runs now progress through `observe -> sop-compact`, produce the durable SOP skill `tiktok-shop-check-inbox-messages` from compact run `20260327_145330_192`, and then start refine with `--skill tiktok-shop-check-inbox-messages`, where refine run `20260327_145552_964` explicitly loads that skill through `skill.reader`. This follow-up slice then closed the remaining handoff bugs: `sandbox-workflow flow/selfcheck` now fails explicitly when compact did not persist a truthful reusable skill, manual `sandbox-workflow refine --skill <name>` is supported, auto-observe terminates the spawned observe runtime when CDP ready wait or demo execution fails, and scripted compact replies are allocated per compact workflow so reply cursors cannot leak across runs. Fresh verification: `node --test .sandbox/bin/*.test.mjs`, `npm --prefix apps/agent-runtime run lint`, `test`, `typecheck`, `build`, and `hardgate`, with fresh hardgate evidence in `artifacts/code-gate/2026-03-27T10-04-24-032Z/report.json`. The remaining problem is not missing SOP context or a homepage URL in the user prompt: the live site currently causes the direct `/chat/inbox/current` navigation to be observed as homepage/message-center instead, so the next refinement slice should improve agent-owned redirect/path-mismatch recovery instead of pushing more URL-handling detail into user prompts.
...
- **Refine TikTok customer-service e2e baseline and prompt/tool clarity pass is complete in this worktree**: TikTok Global Shop customer-service checking is now the active refine e2e baseline, the start prompt explicitly exposes the bootstrap observationRef plus page identity/tab metadata, refine guidance now hard-codes the re-observe rule after navigation/tab changes and treats verified empty inbox states as valid completion, and the key browser/run tool descriptions now mirror those semantics. The business evidence for this slice remains improved TikTok run `20260324_090514_720` in `artifacts/e2e/20260324_090514_720/` (reduced from 17 to 10 assistant turns and from 3 to 1 promoted knowledge versus the first baseline run), while the latest repo-wide green gate evidence for the current worktree is synced separately below.
- **Refine observation stabilization is complete in this worktree**: a real Playwright snapshot corpus is now frozen in test fixtures, the parser now preserves raw tab provenance while deriving `pageTab` / `taskRelevantTabs`, `observe.page` now runs a bounded Hybrid gate before returning a fresh observation, and the only new agent-visible health signal is `observationReadiness = ready | incomplete`. The prompt only teaches that readiness cue conservatively; settle/convergence diagnostics remain internal to the adapter. Latest repo-wide green gate evidence for the current worktree is `artifacts/code-gate/2026-03-24T07-16-04-402Z/report.json`.
- **Refine page-level retrieval cues slice is complete in this worktree**: page-level knowledge matching is now keyed only by exact `origin + normalizedPath`; promoted knowledge has been narrowed to `page + guide + keywords + provenance`; `observe.page` returns `pageKnowledge` straight from the exact-match loader without heuristic rewriting; and the default refine workflow assembly now injects the same loader into the live browser-service path so real runtime runs see the same cues as the focused tests. Fresh verification for this slice: `lint`, `test`, `typecheck`, `build`, and `hardgate` all pass; fresh hardgate evidence is `artifacts/code-gate/2026-03-26T09-45-42-193Z/report.json`.
- **Refine-only TikTok rerun confirms runtime page-knowledge hits**: rerun `20260326_200513_031` completed against the TikTok customer-service refine path and confirmed that `/chat/inbox/current` now receives non-empty `observe.page.pageKnowledge` during a real run. The same run still reports `loadedKnowledgeCount: 0` in `artifacts/e2e/20260326_200513_031/run_summary.json`, which means that field currently measures bootstrap/start-prompt guidance injection only and should not be read as a runtime page-knowledge hit metric.
- **URL literal fidelity remains a known follow-up, not the current P0**: TikTok refine runs on 2026-03-24 repeatedly showed the model corrupting `&register_libra=` into `®ister_libra=` when issuing `act.navigate`. This is tracked as a separate literal-copy bug because the stabilized-observe slice still completes successfully in run `20260324_225753_579`; future navigation debugging should keep this issue separate from observe/tab-ownership regressions.
- **Refine `observe.page` surface analysis is complete in this worktree**: the current observation path is now explicitly documented as a five-layer surface: raw Playwright MCP markdown, adapter metadata parsing, stored observation contract, deterministic `observe.query` search surface, and final agent reasoning. The new analysis note also grounds that model in real TikTok evidence: one successful run where homepage and inbox structures were present in the raw snapshot, and one failed run where the business page was logically correct but the relevant UI regions were missing or weak in the captured tree. Active reference: `docs/project/refine-observe-page-surface-analysis.md`.
- **Refine observation enhancement decision matrix is complete in this worktree**: the follow-up decision note now classifies which problems should be solved in browser capture, adapter structure, query surface, or prompt semantics, and records the current recommendation to treat adapter enrichment as the next highest-leverage layer. It also records the proposed split between raw tab provenance and execution-oriented derived tab views. Active reference: `docs/project/refine-observation-enhancement-decision-matrix.md`.
...
- The active follow-up in this worktree is now to split metric semantics before expanding retrieval any further: keep a startup-loaded guidance count for bootstrap/start-prompt injection, add a runtime page-knowledge hit metric for `observe.page`, and then separately decide whether the start prompt should allow direct finish when stable empty-state knowledge and corroborating DOM evidence agree. The SOP-skill compact->refine slice is now closed; planner-style prompt/SOP disambiguation remains deferred and is not the current P0.
