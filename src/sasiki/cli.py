...
def run_browser_agent(
    task: str = typer.Argument(..., help="Natural language browser task"),
    ...
    cdp_endpoint: Optional[str] = typer.Option(None, "--cdp-endpoint", help="CDP endpoint for Playwright MCP"),
    chrome_profile_dir: Path = typer.Option(Path("~/.sasiki/chrome_profile"), "--chrome-profile-dir"),
    cookies_dir: Path = typer.Option(Path("~/.sasiki/cookies"), "--cookies-dir"),
    ...
    from sasiki.browser import CdpBrowserLauncher, DEFAULT_CDP_ENDPOINT, is_local_cdp_endpoint
    ...
    browser_launcher: Optional[CdpBrowserLauncher] = None
    try:
        if launch_cdp:
